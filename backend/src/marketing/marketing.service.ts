...
export class MarketingService {
    ...
    async createLead(userId: string, data: any) {
        ...
        return this.prisma.lead.create({
            data: { ...data, score, createdBy: userId } });
    }
    ...
    async updateLeadStatus(id: string, userId: string, status: string, notes?: string) {
        await this.ensureLeadExists(id);
        await this.prisma.leadActivity.create({
            data: {
                ...
                createdBy: userId } });
        if (status === 'WON') await this.convertLeadToClient(id, userId);
        return this.prisma.lead.update({
            where: { id },
            data: { status: status as any, updatedAt: new Date() } });
    }

    async logLeadActivity(leadId: string, userId: string, data: any) {
    ...
    async getLeadStats() {
        const [total, newToday, byStatus, bySource, avgScore] = await Promise.all([
            ...
            this.prisma.lead.aggregate({ where: {}, _avg: { score: true } }),
        ]);
    ...
    private async convertLeadToClient(leadId: string, userId: string) {
        ...
        if (lead.affiliateId) {
            ...
            if (affiliate) {
                ...
                await this.prisma.affiliateCommission.create({
                    data: { affiliateId: lead.affiliateId, leadId, amount } });
    ...
    async logCall(userId: string, data: any) {
        return this.prisma.callLog.create({
            data: { ...data, callerUserId: userId } });
    }
    ...
    async createCallScript(data: any) {
        return this.prisma.callScript.create({ data: { ...data } });
    }
    ...
    async getCallStats() {
        ...
        const [todayCalls, answered, avgDuration] = await Promise.all([
            ...
            this.prisma.callLog.count({ where: { createdAt: { gte: today }, status: 'ANSWERED' } }),
            this.prisma.callLog.aggregate({
                where: { createdAt: { gte: today }, duration: { not: null } },
                _avg: { duration: true } }),
        ]);
        return {
            todayCalls,
    ...
    async getMessageCampaigns(type?: string) {
        const where: any = {};
    ...
    async createMessageCampaign(userId: string, data: any) {
        return this.prisma.messageCampaign.create({
            data: { ...data, createdBy: userId } });
    }

    async getMessageCampaignById(id: string) {
    ...
    async getContentCalendar(month: number, year: number) {
