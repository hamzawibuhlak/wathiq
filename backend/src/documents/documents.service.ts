...
export class DocumentsService {
     ...
     */
    async serveFile(id: string, res: Response, disposition: 'inline' | 'attachment' = 'inline') {
    ...
    async download(id: string, res: Response) {
        return this.serveFile(id, res, 'attachment');
    }
    ...
    async upload(
        ...
        // Generate storage path: uploads/{}/{year}/{month}/
        const now = new Date();
        const year = now.getFullYear().toString();
        const month = (now.getMonth() + 1).toString().padStart(2, '0');
        const tenantDir = path.join(this.uploadDir, year, month);
        ...
        try {
            ...
            const document = await this.prisma.document.create({
                data: {
                    ...
                    uploadedById: userId,
                    code: docCode.code,
                    codeNumber: docCode.codeNumber } as any,
                include: {
                    case: { select: { id: true, caseNumber: true, title: true } },
                    uploadedBy: { select: { id: true, name: true } } } });
    ...
    async createNewVersion(
        ...
        const currentDoc = await this.prisma.document.findFirst({
            where: { id: documentId, isLatest: true } });
