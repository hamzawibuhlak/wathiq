...
export class ARService {
    ...
    async createFromInvoice(invoiceId: string) {
        ...
        return this.prisma.accountsReceivable.create({
            data: {
                ...
                balance: invoice.totalAmount, dueDate: invoice.dueDate } });
    }

    async recordPayment(arId: string, dto: {
        amount: number; paymentDate: Date; paymentMethod: any; reference?: string; notes?: string;
    }) {
        const ar = await this.prisma.accountsReceivable.findFirst({ where: { id: arId } });
        ...
        const payNum = await this.generatePaymentNumber();

        const [payment] = await this.prisma.$transaction([
            this.prisma.aRPayment.create({
                data: { arId, paymentNumber: payNum, amount: new Prisma.Decimal(dto.amount), paymentDate: dto.paymentDate, paymentMethod: dto.paymentMethod, reference: dto.reference, notes: dto.notes } }),
    ...
    private async generatePaymentNumber() {
        ...
        const count = await this.prisma.aRPayment.count({ where: { paymentNumber: { startsWith: `PAY-${year}` } } });
